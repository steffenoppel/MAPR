...
cat("
  ...
  model {
    ...
    mean.fec ~ dunif(0,0.2)         ## uninformative prior for CURRENT FECUNDITY
    orig.fec ~ dunif(0.155,0.165)         ## uninformative prior for ORIGINAL FECUNDITY
    full.fec ~ dnorm(0.519,100) T(0.1,0.52)     ## prior for full fecundity without predation from Nevoux & Barbraud (2005) - very high precision
...
jags.data <- list(## survival
  ...
  ## population process
  #Ntot.breed=matrix(NA,nrow=100,ncol=2),
  PROJ=100)
#jags.data$Ntot.breed[46,1:2]<-800000
#jags.data$Ntot.breed[1,1:2]<-4500000
...
parameters <- c("orig.fec","mean.fec","full.fec","fec.decrease","juv.surv","phi","p","growth.rate","lambda","Ntot.breed")
...
MAPR_IPM <- autojags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\MAPR\\MAPR_IPM_v4.jags",
...
TABLE1<-out %>% filter(parameter %in% c('orig.fec','mean.fec','full.fec','fec.decrease','phi','juv.surv','growth.rate[1]','growth.rate[2]')) %>%
  select(parameter,c(5,3,7))

names(TABLE1)<-c("Parameter","Median","lowerCL","upperCL")
TABLE1$Parameter<-c("original fecundity","current fecundity","mouse-free fecundity","fecundity decrease","first year survival","adult survival","population growth rate (no eradication)","population growth rate (with eradication)")
TABLE1
fwrite(TABLE1,"MAPR_demographic_parameter_estimates_v4.csv")
...
ggplot()+
  ...
  ## format axis ticks
  scale_y_continuous(name="N MacGillivray's Prion on Gough (1000s)", limits=c(0,5000000),breaks=seq(0,5000000,500000),labels=seq(0,5000,500))+
...
ggsave("MAPR_population_projection_v4_CI50.jpg", width=9, height=6)
